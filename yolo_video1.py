    ...
    if len(boxes) > 0:
        ...
        if len(indices) > 0:
            ...
            if density_percentage <= density_threshold and not density_threshold_reached:
                # This block handles when the density is below the threshold and the flag is not set
                start_time = time.time()
                density_threshold_reached = True  # Set the flag immediately to avoid redundant calculations
            ...
            if density_threshold_reached:
                # This block handles when the density is above the threshold
                elapsed_time = time.time() - start_time
                multiplied_time = elapsed_time * 3

                print(f"Time to reach density threshold: {elapsed_time:.2f} seconds (multiplied by 3: {multiplied_time:.2f} seconds)")
                print(f"Density at that time: {density_percentage:.2f}%")
